...
var storage = multer.diskStorage({
    destination: function(req, file, callback) {
        callback(null, './uploads') // 上传的文件夹目录
    },
...
exports.filedata = function(req, res, next) {
    ...
    if (id !== 'null') {
        var upload = multer({
            ...
            .single('avatar'); // single是单文件上传,跟前端名称对应
        upload(req, res, function(err) { // 直接调用upload
...
exports.remove = function(req, res, next) {
    var id = req.params.id;
    Upload.findByIdAndRemove(id, function(err, doc) { // 除了删除数据，还需要注意要删除文件
...
exports.removes = function(req, res, next) {
    ...
    if (ids.length > 0) {
        Upload.remove({ _id: { $in: ids } }).then(deletedData => {
            res.json({ "message": "delete ok" });
        });
    } else {
